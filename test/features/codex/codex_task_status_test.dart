...
void main() {
  ...
  test(
    'uses rollout modification time without reading rollout contents',
    () async {
      ...
      try {
        final rollout = File(
          '${root.path}${Platform.pathSeparator}rollout-test.jsonl',
        );
        await rollout.writeAsString('not parsed by the status adapter');
        final now = DateTime(2026, 9, 14, 12);
        await rollout.setLastModified(
  ...
  test(
    'shows completed after the active window and hides stale sessions',
    () async {
      final root = await Directory.systemTemp.createTemp(
        'flclash-task-status-',
      );
  ...
  test('returns unavailable when the sessions directory is missing', () async {
    final reader = CodexTaskStatusReader(
      sessionsRoot:
          '${Directory.systemTemp.path}${Platform.pathSeparator}missing-flclash-sessions',
    );
